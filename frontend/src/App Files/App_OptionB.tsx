...
import { DrugCard, getDrug, searchDrugs } from './lib/api';
...
type ExecutiveMedicationScorecardProps = {
  drug: any | null;
};
...
function formatScore(value: unknown, options?: { percent?: boolean; decimals?: number }) {
  const numeric = asNumber(value);

  if (numeric === null) return '—';

  const decimals = options?.decimals ?? (numeric % 1 === 0 ? 0 : 1);
...
function getClassificationCount(classifications: Record<string, any>) {
...
function ExecutiveMedicationScorecard({ drug }: ExecutiveMedicationScorecardProps) {
  if (!drug) return null;
  ...
  const tty = drug.tty || drug.drug?.tty;
  ...
  const totalClassifications =
    ...
    getClassificationCount(classifications);
  ...
  const relationshipCount =
    drug.relationship_count ||
    drug.relationships_count ||
    drug.rxnorm_relationship_count ||
    (Array.isArray(relationships) ? relationships.length : 0) ||
    (Array.isArray(relationships?.items) ? relationships.items.length : 0) ||
    (Array.isArray(relationships?.edges) ? relationships.edges.length : 0);
  ...
  return (
    <section className="rounded-[2rem] border border-blue-900/50 bg-slate-950 p-6 text-white shadow-2xl shadow-blue-950/20">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-3">
            <h2 className="text-4xl font-black tracking-tight text-white">{name}</h2>
            {tty && (
              <span className="rounded-full border border-slate-700 bg-slate-900 px-3 py-1 text-xs font-black text-slate-100">
                {tty}
              </span>
            )}
            <span className="rounded-full border border-amber-300 bg-amber-100 px-4 py-1 text-xs font-black text-amber-900">
              {tier}
            </span>
          </div>
          {rxcui && <p className="mt-2 text-sm font-semibold text-slate-400">RxCUI {rxcui}</p>}
        </div>

        <div className="rounded-3xl border border-slate-800 bg-slate-900 px-6 py-5 text-left lg:min-w-64 lg:text-right">
          <p className="text-xs font-black uppercase tracking-[0.22em] text-slate-400">
            Overall Intelligence
          </p>
          <p className="mt-2 text-5xl font-black leading-none text-white">
            {formatScore(overall, { percent: true, decimals: 0 })}
          </p>
          <p className="mt-2 text-sm font-bold text-slate-300">
            {formatScore(overall, { decimals: 2 })} raw score
          </p>
        </div>
      </div>

      <div className="mt-6 grid gap-4 md:grid-cols-4">
        <ExecutiveMetric label="Claims Readiness" value={formatScore(claims, { percent: true, decimals: 0 })} />
        <ExecutiveMetric label="AI Readiness" value={formatScore(ai, { percent: true, decimals: 0 })} />
        <ExecutiveMetric label="Semantic Richness" value={formatScore(semantic, { percent: true, decimals: 0 })} />
        <ExecutiveMetric label="Interoperability" value={formatScore(interoperability, { percent: true, decimals: 0 })} />
      </div>

      <div className="mt-6 rounded-3xl border border-slate-800 bg-white p-5 text-slate-950">
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-5">
          <ExecutiveMetric
            ...
            value={formatMetric(totalClassifications)}
            helper="Mapped evidence records"
          />
          <ExecutiveMetric
            label="Domains"
            value={formatMetric(domainCount)}
            helper="Populated intelligence domains"
          />
          <ExecutiveMetric
            label="Disease Maps"
            value={formatMetric(diseaseCount)}
            helper="Clinical interpretation signals"
          />
          <ExecutiveMetric
            label="Relationships"
            value={formatMetric(relationshipCount)}
            helper="RxNorm linked concepts"
          />
          <ExecutiveMetric
            label="ATC Depth"
            value={`${formatMetric(atcDepth)}/4`}
            helper="Hierarchy coverage"
          />
        </div>
      </div>
    </section>
    ...
    <main className="min-h-screen bg-slate-950 text-white">
      ...
      {activeDrug && (
        <section className="mx-auto max-w-7xl space-y-8 px-6 py-8">
          <ExecutiveMedicationScorecard drug={drugWithDetails} />
          ...
          {activeDashboardPage === 'overview' && null}
