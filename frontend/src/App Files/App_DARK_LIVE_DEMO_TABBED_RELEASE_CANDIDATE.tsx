...
import { Activity, Brain, Database, FlaskConical, GitBranch, Layers, Network, Rocket, Search, ShieldCheck, Sparkles, Target } from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function runSearch() {
      setLoading(true);
      try {
        const term = query.trim() || 'semaglutide';
        const data = await searchDrugs(term, 30);
        if (!cancelled) {
          const nextResults = Array.isArray(data) ? data : [];
          setResults(nextResults);
          if (!selected && nextResults.length > 0) {
        ...
        <section className="mx-auto max-w-7xl space-y-8 px-6 py-8">
          ...
          {activeDashboardPage === 'overview' && (
            ...
            >
              ...
              <SafeCardBoundary title="Medication Intelligence Summary">
                <IntelligenceCard drug={activeDrug} />
              </SafeCardBoundary>

              <SafeCardBoundary title="Medication Intelligence Profile">
                <MedicationIntelligenceSummaryCard drug={activeDrug} />
              </SafeCardBoundary>
          ...
          {activeDashboardPage === 'atc' && (
            ...
            >
              ...
              <div className="grid gap-4 md:grid-cols-4">
                <MiniStat label="ATC Depth" value={(activeDrug as any).atc_depth || (activeDrug as any).atc_hierarchy_depth || '—'} />
                <MiniStat label="Classifications" value={(activeDrug as any).classification_count || (activeDrug as any).total_classifications || '—'} />
                <MiniStat label="Class Types" value={(activeDrug as any).class_type_count || '—'} />
                <MiniStat label="Clinical Signals" value={(activeDrug as any).disease_count || '—'} />
              </div>
            </PageShell>
          )}
