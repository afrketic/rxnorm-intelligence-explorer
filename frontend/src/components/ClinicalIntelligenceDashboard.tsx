...
function getDiseaseBurdenForecast(drug: any) {
  const forecast =
    drug?.disease_burden_forecast ||
    drug?.drug?.disease_burden_forecast ||
    drug?.clinical_briefing?.disease_burden_forecast ||
    {};
  ...
  return {
    ...
    signal: signal || 'Not Available',
    diseaseDomain: diseaseDomain || 'Disease trend not yet mapped',
    narrative:
      narrative ||
      'Disease burden forecasting will populate after H4B.3 creates disease_burden_forecasting_v1.',
    currentPrevalence: forecast?.current_prevalence,
    currentMortality: forecast?.current_mortality,
    populationBurdenTier: forecast?.population_burden_tier,
  };
}

function formatPrevalence(value: unknown) {
  const numeric = Number(value);
  if (!Number.isFinite(numeric)) return 'Available evidence';
  return `${numeric.toFixed(1)}% prevalence`;
}
...
export default function ClinicalIntelligenceDashboard({ drug, onSelectSimilarDrug }: Props) {
  ...
  const diseaseBurdenForecast = getDiseaseBurdenForecast(drug);
...
<section className="space-y-6 text-white">
  ...
  <section className="rounded-[2rem] border border-amber-300/20 bg-slate-950/70 p-6 shadow-[0_0_24px_rgba(245,158,11,0.12)]">
    <p className="text-xs font-black uppercase tracking-[0.24em] text-amber-300">
      Forward-Looking Disease Intelligence
    </p>

    <div className="mt-2 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
      <div>
        <h3 className="text-2xl font-black text-white">Population Burden Trend</h3>
        <p className="mt-2 max-w-4xl text-sm font-semibold leading-6 text-slate-300">
          {diseaseBurdenForecast.narrative}
        </p>
      </div>

      <span className="w-fit rounded-2xl border border-amber-300/30 bg-amber-400/10 px-4 py-3 text-sm font-black text-amber-100">
        {diseaseBurdenForecast.signal}
      </span>
    </div>

    <div className="mt-5 grid gap-4 md:grid-cols-3">
      <div className="rounded-2xl border border-white/10 bg-slate-950/60 p-4">
        <p className="text-xs font-black uppercase tracking-[0.16em] text-amber-300">Disease Area</p>
        <p className="mt-2 text-lg font-black text-white">{diseaseBurdenForecast.diseaseDomain}</p>
      </div>
      <div className="rounded-2xl border border-white/10 bg-slate-950/60 p-4">
        <p className="text-xs font-black uppercase tracking-[0.16em] text-amber-300">Current Prevalence</p>
        <p className="mt-2 text-lg font-black text-white">{formatPrevalence(diseaseBurdenForecast.currentPrevalence)}</p>
      </div>
      <div className="rounded-2xl border border-white/10 bg-slate-950/60 p-4">
        <p className="text-xs font-black uppercase tracking-[0.16em] text-amber-300">Burden Direction</p>
        <p className="mt-2 text-lg font-black text-white">{diseaseBurdenForecast.signal}</p>
      </div>
    </div>
  </section>
