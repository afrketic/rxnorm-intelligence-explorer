...
def suggest_drug_names(query: str, max_results: int = 8):
    """
    Autocomplete prefers the local monthly RxNorm lightweight index.
    If the index is unavailable or has no matches, it falls back to the
    live RxNav suggestion helper.
    """
    safe_max = max(1, min(int(max_results or 8), 25))
    indexed = search_rxnorm_index(query, limit=safe_max)

    if indexed.get("success") and indexed.get("suggestions"):
        # Frontend autocomplete expects a simple list of display strings.
        # Keep the richer indexed records available for diagnostics.
        indexed_records = indexed.get("suggestions", [])
        return {
            **indexed,
            "suggestions": [record.get("name", "") for record in indexed_records if record.get("name")],
            "suggestion_records": indexed_records,
        }

    live = get_drug_name_suggestions(query, max_results=safe_max)
    if isinstance(live, dict):
        live["served_from_rxnorm_index"] = False
        live["index_fallback_reason"] = indexed.get("message", "No indexed suggestions were available.")
    return live
