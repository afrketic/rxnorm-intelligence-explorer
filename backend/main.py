...
try:
    from backend.rxnorm_intelligence_api_v1 import get_drug_name_suggestions
except ImportError:
...
try:
    from backend.rxnorm_bulk_index import (
        build_or_refresh_rxnorm_index,
        get_rxnorm_index_stats,
        search_rxnorm_index,
    )
except ImportError:
    try:
        from rxnorm_bulk_index import (
            build_or_refresh_rxnorm_index,
            get_rxnorm_index_stats,
            search_rxnorm_index,
        )
    except ImportError:
        def build_or_refresh_rxnorm_index(force: bool = False):
            return {
                "success": False,
                "status": "unavailable",
                "message": "RxNorm bulk index builder is unavailable. Confirm backend/rxnorm_bulk_index.py exists and is deployed.",
                "force": force,
            }

        def get_rxnorm_index_stats():
            return {
                "success": False,
                "status": "unavailable",
                "message": "RxNorm bulk index status is unavailable. Confirm backend/rxnorm_bulk_index.py exists and is deployed.",
            }

        def search_rxnorm_index(query: str, limit: int = 8):
            return {
                "success": False,
                "query": query,
                "suggestions": [],
                "message": "RxNorm lightweight index is unavailable.",
            }
...
@app.post("/rxnorm-index/build")
def build_rxnorm_index_endpoint(force: bool = False):
    """
    Builds or refreshes the lightweight all-drugs RxNorm index.

    The index is intended to persist all RxNorm drug identities at the lightweight
    search/autocomplete level while keeping expensive NDC and graph payloads lazy
    and disk-cached on first use.
    """
    return build_or_refresh_rxnorm_index(force=force)


@app.get("/rxnorm-index/status")
def rxnorm_index_status_endpoint():
    """Returns current status and file diagnostics for the lightweight RxNorm index."""
    return get_rxnorm_index_stats()
