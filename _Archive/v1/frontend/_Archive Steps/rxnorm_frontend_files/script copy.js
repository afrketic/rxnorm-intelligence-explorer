...
function renderIdentityCard(data) {
  document.getElementById("searchTerm").textContent =
    data.identity_card?.search_term || "";

  document.getElementById("primaryRxcui").textContent =
    data.identity_card?.primary_rxcui || "";

  document.getElementById("conceptName").textContent =
    data.identity_card?.concept_name || "";
...
function renderInteroperabilityFlow(data) {
  ...
  const nodes =
    data.interoperability_graph?.interoperability_flow?.nodes || [];
...
function renderTherapeuticClasses(data) {
  ...
  atcItems.slice(0, 12).forEach((item) => {
    const div = document.createElement("div");
...
function renderNdcCrosswalk(data) {
  ...
  ndcItems.slice(0, 20).forEach((item) => {
    const div = document.createElement("div");
    div.className = "list-item";
    div.innerHTML = `
      <strong>NDC11:</strong> ${item.ndc11 || "N/A"}
      | <strong>NDC10:</strong> ${item.ndc10 || "N/A"}
      | <strong>NDC9:</strong> ${item.ndc9 || "N/A"}
      | <strong>Clinical RxCUI:</strong> ${item.clinical_drug_rxcui || item.rxcui || "N/A"}
    `;
    container.appendChild(div);
  });
}