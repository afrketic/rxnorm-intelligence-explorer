...
# =========================================================
# DRUG NAME AUTOCOMPLETE / TYPEAHEAD SUPPORT
# =========================================================

def get_drug_name_suggestions(query: str, max_results: int = 8) -> Dict[str, Any]:
    """
    Returns RxNorm/RxNav medication name suggestions for the frontend
    autocomplete/typeahead search box.

    The frontend calls this while the user types. The function uses the
    RxNav approximateTerm endpoint so misspellings or partial terms can still
    return useful medication-name candidates.
    """
    ...
    if data:
        ...
        for item in candidate_items:
            candidate_name = preserve_leading_zeros(item.get("name", "")).strip()
            if candidate_name:
    ...
    return {
        ...
        "source": "RxNorm / RxNav approximateTerm API"
    }
